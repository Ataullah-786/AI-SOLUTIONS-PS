...
> block below is currently **~7,998**, which is effectively at the ceiling. Do not add to it.
...
Product Registry, and confirm the product and table are supported there before fetching. Then
run the validation passes, report using the orchestrator's severity classification, remediate
every issue classed `AUTO-CORRECTABLE`, and return the cleaned file, Change Log and
...
the value as-is, log it as `Unresolved`, and state what the user must supply.**Your deliverables when remediation runs**
...
- `{original-name}_CHANGELOG.csv` — one row per change and per unresolved issue: Row, Column,
  ...
  changed`), Reason, Source, Status. Never `README.md` or the rules file.- `{original-name}_VALIDATION_REPORT.md` — initial status, findings, final status after
...
save, and never reuse the most recent tool response as file content because it is the
nearest block of text.
...
identify the product or table, ask — do not guess.
...
5. **+ New step** → **Respond to the agent** → **+ Add an output** → **Text** → name it
   ...
   **Do not skip this step.** It is what returns the link to the agent so the user can open
   ...
   `/Shared Documents/Report_CLEANED.csv` — it is not clickable. `Link to item` is the full
